...
if [ "$MODE" = "-metal" ]; then
    clang++ "$@" -x objective-c++ bento/metal/metal.mm bento/metal/metaltexture.mm -Wno-null-character -std=c++17 -framework Metal -framework QuartzCore -framework Cocoa -framework IOKit -framework MetalKit -framework CoreVideo -framework Foundation -framework CoreFoundation -framework Carbon -o "$OUTPUT" -DUSE_METAL || exit 1 
...
codesign -s - -v -f --entitlements debug.plist ../bentoextras/bento
