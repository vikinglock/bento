...
if [ "$fw" = "-metal" ]; then
    ...
    clang++ "$@" -x objective-c++ -o "$output" bento/metal/metal.mm bento/metal/metaltexture.mm bento/lib/hidapi/mac/hid.c -std=c++17\
                    ...
                    -framework Metal -framework QuartzCore -framework Cocoa -framework IOKit -framework MetalKit -framework CoreVideo -framework Foundation -framework CoreFoundation -framework Carbon -framework OpenAL -Wno-null-character -std=c++17 -DUSE_METAL || exit 1 
elif [ "$fw" = "-opengl" ]; then

    if [ "$output" = "-convert" ]; then
        echo "(converting shaders to glsl 3.3)"
    ...
    clang++ "$@" bento/opengl/opengl.cpp bento/opengl/opengltexture.cpp glad.o\
                 ...
                 -std=c++17 -framework OpenGL -framework QuartzCore -framework Cocoa -framework IOKit -framework CoreVideo -framework CoreFoundation -framework Carbon -framework OpenAL -lglfw3 -o "$output" -DUSE_OPENGL || exit 1
    rm glad.o
else
    echo "Usage: $0 (-metal|-opengl) <output_name> <c++ file> <another c++ file> ..."
    exit 1
