...
DBVT_INLINE int btDbvtAabbMm::Classify(const btVector3& n, btScalar o, int s) const
{
	...
	switch (s)
	{
		case (0 + 0 + 0):
			...
			pq = btVector3(mx.x(), mx.y(), mx.z());
			break;
		case (1 + 0 + 0):
			px = btVector3(mx.x(), mi.y(), mi.z());
			pq = btVector3(mi.x(), mx.y(), mx.z());
			break;
		case (0 + 2 + 0):
			px = btVector3(mi.x(), mx.y(), mi.z());
			pq = btVector3(mx.x(), mi.y(), mx.z());
			break;
		case (1 + 2 + 0):
			px = btVector3(mx.x(), mx.y(), mi.z());
			pq = btVector3(mi.x(), mi.y(), mx.z());
			break;
		case (0 + 0 + 4):
			px = btVector3(mi.x(), mi.y(), mx.z());
			pq = btVector3(mx.x(), mx.y(), mi.z());
			break;
		case (1 + 0 + 4):
			px = btVector3(mx.x(), mi.y(), mx.z());
			pq = btVector3(mi.x(), mx.y(), mi.z());
			break;
		case (0 + 2 + 4):
			px = btVector3(mi.x(), mx.y(), mx.z());
			pq = btVector3(mx.x(), mi.y(), mi.z());
			break;
		case (1 + 2 + 4):
			px = btVector3(mx.x(), mx.y(), mx.z());
			pq = btVector3(mi.x(), mi.y(), mi.z());
			break;
	}
	if ((btDot(n, px) + o) < 0) return (-1);
	if ((btDot(n, pq) + o) >= 0) return (+1);
	return (0);
}
